...
import framework.model.DefaultTransaction;
...
public class DepositFrame<T extends Account> implements Frame {

    private final Database<T> database;
    private final JFrame jFrame;
    String accountNumber;

    public DepositFrame(Database<T> database, String accountNumber) {
    ...
    public class SymAction implements java.awt.event.ActionListener {
        public void actionPerformed(java.awt.event.ActionEvent event) {
            ...
            if (object == JButton_OK)
                ...
                JButtonCalcel_actionPerformed();
        }
    }

    void JButtonOK_actionPerformed() {
        int amount = Integer.parseInt(JTextField_Deposit.getText());
        database.getAccount(accountNumber).addTransaction(new DefaultTransaction("default payment",amount, Utils.getCurrentTime()));
        database.notify(database.getAccountList());
        dispose();
    }

    void JButtonCalcel_actionPerformed() {
        dispose();
    }
